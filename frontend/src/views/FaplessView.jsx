...
export default function FaplessView() {
  ...
  if (loading) return (
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', gap: 10, flexDirection: 'column' }}>
      ...
      <style>{`@keyframes pulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:0.6;transform:scale(0.95)} } @keyframes spin { to { transform: rotate(360deg) } } @keyframes shimmer { 0%{background-position:-200% 0} 100%{background-position:200% 0} } @keyframes glow { 0%,100%{box-shadow:0 0 10px ${currentLevel?.color || '#ff4500'}44} 50%{box-shadow:0 0 30px ${currentLevel?.color || '#ff4500'}88} }`}</style>
    </div>
  )

  // After relapse — show quote
  if (relapseQuote) return (
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 24, padding: 40 }}>
      ...
      <div style={{
        maxWidth: 560, textAlign: 'center', fontSize: 20, fontWeight: 700, lineHeight: 1.6,
